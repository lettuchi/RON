...
## Case 4 — Canal supper (宴の呼び声) — vertical slice

**Hook:** Chrysanthemum factor invites Kaoru to a public canal pavilion supper; tests whether Toa stays in the hall as witness or decoration.

**Setpiece:** Table placement — stand beside his chair, defer to lower cushion, or walk out.

**Romance:** Defend the magistrate's witness seat → `case4_kaoru_defended` → `case4_closed`.

**Bad end:** Walk out alone → **Game Over** 「宴の呼び声」 (`case4_bad_end_abandoned`).

**Post-close (full romance run):** `all_romance_routes_complete()` → **"Is this a date?"** (`case4_is_this_a_date_interlude`) — canal walk, skewers, deflection, almost-admission walked back. No bad end. Design: `docs/case4-date-interlude-design.md`.

*Note:* Dock swordfight CGs/labels (`case4_toa_slain`, `gameover_case4_slayn`) remain in `game/images/cgs-case4.rpy` for a future dock raid beat; not wired in the current supper script.
...
| `case4_started` / `case4_closed` | Case Four supper vertical slice |
| `case4_kaoru_defended` | Table defend (date interlude gate) |
| `case4_bad_end_abandoned` | Supper walk-out bad end |
| `case4_date_interlude_seen` | "Is this a date?" interlude viewed |
| `is_this_a_date_asked` | Player pressed the date question |
| `case4_toa_slain` | Reserved — dock duel bad end (future) |
